...
public abstract class Graph {
	...
	public List<Integer> degreeSequence() {
		...
		for(int i=0;i<getNumVertices();i++){
			degree=0;
			for(int d : this.getNeighbors(i)){
				degree++;
			}
			for(int d : this.getInNeighbors(i)){
				degree++;
			}
			dS.add(degree);
		}
	...
	public static void main (String[] args) {
		...
		GraphAdjMatrix graphMatrix = new GraphAdjMatrix();
		GraphLoader.loadRoadMap("data/testdata/simpletest.map", graphMatrix);
		System.out.println(graphMatrix);
		
		System.out.println(graphMatrix.getDistance2(3));
